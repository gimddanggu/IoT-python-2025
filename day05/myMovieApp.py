...
def run():
    ...
    while True:
        ...
        if sel_menu == 1:
            ...
            print('영화검색')
            search_title = input('검색할 영화명 입력> ')
            search_movie(lst_movie, search_title)

        elif sel_menu == 4:
            print('영화삭제')
            del_title = input('삭제할 영화명 입력> ')
            del_movie(lst_movie, del_title)
        ...
        input() # 입력대기
        clearScreen() # 메누 기능이 완료되면 화면 클리어
