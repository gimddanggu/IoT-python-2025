...
class Block:
    ...
    def move(self):  #볼 무브
        ...
        # 볼이 움직이는 y축 값을 계속 계산하려면 y dir 값을 라디언으로 변환 후 코사인 처리
...
def main():
    ...
    score = 0
    BLOCK = []
    BALL = Block((200, 200, 0), Rect(375, 620, 20, 20), 10) # 공 생성
    ...
    while True:
        ...
        if is_game_start == False:
            ...
            Surface.blit(M_GAME_SUBTITLE, ((SCREEN_WIDTH // 2) - (300 // 2),
                         (SCREEN_HEIGHT // 2) - (50 // 2)+70))
            
        else: # 게임시작 후 블록 다 그리고 볼이 움직이게 처리, 바도 움직이도록 처리
            Surface.blit(M_SCORE, (20, 770))
            Surface.blit(M_SPEED, (SCREEN_WIDTH- 200, 770))
            ...
            if len(BLOCK) != LenBlock: # 공에 맞아서 블럭이 사라짐
            ...
            if BALL.rect.centerx < 10 or BALL.rect.centerx > (SCREEN_WIDTH-10): # 게임회면 양쪽 벽 밖으로 못 나가게
            ...
            # 게임 클리어, 종료 로직
            if len(BLOCK) == 0:
                Surface.blit(M_CLEAR, ((SCREEN_WIDTH // 2) - (200 // 2),
                         (SCREEN_HEIGHT // 2) - (50 // 2)))
            if BALL.rect.centery > 800:
                Surface.blit(M_FAIL, ((SCREEN_WIDTH // 2) - (200 // 2),
                         (SCREEN_HEIGHT // 2) - (50 // 2)))
                # is_game_start = False # 게임종료 후 재시작은 나중에 다시!!
